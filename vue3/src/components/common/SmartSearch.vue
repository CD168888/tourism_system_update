<template>
  <div class="smart-search" ref="searchContainer">
    <el-input v-model="searchKeyword" :placeholder="placeholder" size="large" class="search-input" @input="handleInput"
      ...
      <template #suffix>
        <el-button type="primary" @click="handleSearch" class="search-btn" :loading="searching">
          搜索
        </el-button>
      </template>
    </el-input>
    ...
    <!-- 搜索建议下拉框 -->
    <div v-show="showSuggestions && (suggestions.scenics.length > 0 || suggestions.guides.length > 0)"
      class="suggestions-dropdown" @mousedown.prevent>
      <!-- 景点建议 -->
      <div v-if="suggestions.scenics.length > 0" class="suggestion-section">
        ...
        <div v-for="(item, index) in suggestions.scenics" :key="`scenic-${item.id}`" class="suggestion-item"
          :class="{ active: selectedIndex === index }" @click="goToScenicDetail(item.id)"
      ...
      <div v-if="suggestions.guides.length > 0" class="suggestion-section">
        ...
        <div v-for="(item, index) in suggestions.guides" :key="`guide-${item.id}`" class="suggestion-item"
          :class="{ active: selectedIndex === (suggestions.scenics.length + index) }" @click="goToGuideDetail(item.id)"
...
.search-input {
  ...
  .search-btn {
    border-radius: 50px;
    background: linear-gradient(45deg, #67b6f5, #5aa9e6);
    border: none;
    padding: 12px 24px;
    font-weight: 600;
    box-shadow: 0 4px 15px rgba(103, 182, 245, 0.4);
    ...
    &:hover {
      ...
      box-shadow: 0 6px 20px rgba(103, 182, 245, 0.6);
